...
def get_poi_id_to_closest_street_id_dict(conn, args):
	...
	***			  road id as its value.
	...
	sql = "select {0}.id as poi_id, {0}.class_code, {0}.geom from {0}".format(args["pois_tbl_name"])
...
def get_street_id_to_closest_pois_boolean_and_counts_per_label_dict(conn, threshold, args):
	...
	*** whether they are located within threshold distance of the street
	...
	for index, row in df_edges.iterrows():
		# for each poi find its distance to all the roads in the dataset
		sql = "select {0}.id as poi_id, {1}.id as edge_id, {0}.geom, ST_Distance(ST_Transform({0}.geom, 32634), {1}.geom) as dist from {0}, {1} where {1}.id = {2}".format(args["pois_tbl_name"], args["roads_tbl_name"], row['edge_id'])
		dist_df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
...
def get_closest_pois_boolean_and_counts_per_label_streets(conn, args, threshold = 0):
	
	# get the dictionary mapping each poi id to that of its closest road
	poi_id_to_closest_street_id_dict = get_poi_id_to_closest_street_id_dict(conn, args)
...
def get_poi_id_to_boolean_and_counts_per_class_dict(conn, num_of_labels, poi_id_to_encoded_labels_dict, threshold, args):
	...
	***				threshold: the aforementioned threshold
	...
	sql = "select {0}.id as poi_id, geom, x, y from {0}".format(args["pois_tbl_name"])
	df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
...
def get_closest_pois_boolean_and_counts_per_label(conn, args, threshold = 0):
	...
	*** 			which is below the given threshold
	...
	return get_poi_id_to_boolean_and_counts_per_class_dict(conn, len(encoded_labels_set), poi_id_to_encoded_labels_dict, threshold, args)
