...
def get_area_mxm(conn, args):
	...
	sql = "select ST_Area(geom) as mxm, geom from {0}".format(args["tbl_name"])
...
def get_perimeter(conn, args):
	...
	sql = "select ST_Perimeter(geom) as perimeter, geom from {0}".format(args["tbl_name"])
	df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
...
def get_vertices(conn, args):
	...
	sql = "select ST_Perimeter(geom) as num_of_vertices, geom from {0}".format(args["tbl_name"])
	df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
		
	return df['num_of_vertices']
...
def get_touches(conn, args):
	...
	sql = "select id, geom from {0}".format(args["tbl_name"])
	df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
	...
	for index, row in df.iterrows():
		...
		sql_touches = "SELECT * FROM {0} WHERE ST_Touches({0}.geom,(SELECT {0}.geom FROM {0} WHERE {0}.id = {1}))".format(args["tbl_name"], row['id'])
...
def get_instersects(conn, args):
	...
	for index, row in df.iterrows():
		...
		sql_touches = "SELECT * FROM {0} WHERE ST_Intersects({0}.geom,(SELECT {0}.geom FROM {0} WHERE {0}.id = {1}))".format(args["tbl_name"], row['id'])
		df_touches = gpd.GeoDataFrame.from_postgis(sql_touches, conn)
		for index, row in df_touches.iterrows():
			id_dictionary[row['id']][0] = 1
			id_dictionary[row['id']][1] += 1
...
def get_covers(conn, args):
	...
	for index, row in df.iterrows():
		...
		sql_touches = "SELECT * FROM {0} WHERE ST_Covers({0}.geom,(SELECT {0}.geom FROM {0} WHERE {0}.id = {1}))".format(args["tbl_name"], row['id'])
		df_touches = gpd.GeoDataFrame.from_postgis(sql_touches, conn)
		for index, row in df_touches.iterrows():
			id_dictionary[row['id']][0] = 1
			id_dictionary[row['id']][1] += 1
...
def get_coveredbys(conn, args):
	...
	for index, row in df.iterrows():
		...
		sql_touches = "SELECT * FROM {0} WHERE ST_CoveredBy({0}.geom,(SELECT {0}.geom FROM {0} WHERE {0}.id = {1}))".format(args["tbl_name"], row['id'])
		df_touches = gpd.GeoDataFrame.from_postgis(sql_touches, conn)
		for index, row in df_touches.iterrows():
			id_dictionary[row['id']][0] = 1
			id_dictionary[row['id']][1] += 1
