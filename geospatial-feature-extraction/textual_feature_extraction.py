...
def get_corpus(conn, args, n_grams = False):
	...
	# get all poi details
	sql = "select {0}.id as poi_id, {0}.name_u as name, {0}.geom from {0}".format(args["pois_tbl_name"])
...
def get_poi_top_k_features(conn, top_k_features, args):
	# get all poi details
	sql = "select {0}.id as poi_id, {0}.name_u as name, {0}.geom from {0}".format(args["pois_tbl_name"])
	df = gpd.GeoDataFrame.from_postgis(sql, conn, geom_col = 'geom')
...
def get_features_top_k(conn, args):
...
def get_features_top_k_ngrams(conn, args):
	...
	# get corpus
	corpus = get_corpus(conn, args, n_grams = True)
